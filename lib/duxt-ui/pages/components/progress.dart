import 'package:jaspr/jaspr.dart';
import 'package:jaspr/dom.dart';
import 'package:duxt_ui/duxt_ui.dart';
import '../../../shared/components/app_scaffold.dart';
import '../../../shared/components/sidebar_ui.dart';
import '../../../shared/components/code_preview.dart';

class ProgressPage extends StatelessComponent {
  const ProgressPage({super.key});

  @override
  Component build(BuildContext context) {
    return AppScaffold(
      sidebar: SidebarUi(),
      body: Component.fragment([
            h1([Component.text('Progress')]),
            p(classes: 'text-xl text-zinc-500 dark:text-zinc-400', [Component.text('Visual indicators for task completion.')]),

            _section('Basic Usage', [
              ComponentPreviewCard(preview: [div(classes: 'w-full max-w-sm', [DProgress(value: 50)])], code: 'DProgress(value: 50)'),
            ]),

            _section('Values', [
              ComponentPreviewCard(preview: [
                div(classes: 'w-full max-w-sm space-y-4', [
                  DProgress(value: 25),
                  DProgress(value: 50),
                  DProgress(value: 75),
                  DProgress(value: 100),
                ]),
              ], code: '''DProgress(value: 25)
DProgress(value: 50)
DProgress(value: 75)
DProgress(value: 100)'''),
            ]),
          ]),
    );
  }

  Component _section(String title, List<Component> children) => div(classes: 'my-8', [h2(classes: 'text-xl font-semibold text-zinc-900 dark:text-white mb-4', [Component.text(title)]), ...children]);
}
